...
def create(path: str, name: str, header: dict, traces: List[Trace], lims: list) -> Figure:
    ...
    for i, tr in enumerate(traces):
        ...
        if len(axes_gyro):
            axes_gyro[0].get_shared_x_axes().joined(axes_gyro[0], ax0)
        ...
        if len(axes_debug):
            axes_debug[0].get_shared_x_axes().joined(axes_debug[0], ax1)
        ...
        if i < 2:
            ...
            if len(axes_d):
                axes_d[0].get_shared_x_axes().joined(axes_d[0], ax2)
            axes_d.append(ax2)
            ...
            axes_d[0].get_shared_x_axes().joined(axes_d[0], ax21)
        ...
        if len(axes_trans):
            axes_trans[0].get_shared_x_axes().joined(axes_trans[0], ax3)
